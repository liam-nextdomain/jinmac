...
//
// Moja(../Moja/scripts/kb.swift)에서 이식했다. 바꾼 곳은 문서 이름표, 요구사항 식별자(F-xx, A-xx),
// 범위 표기, 로드맵 단계(P0~P4), 절 참조 단서의 하이픈 경계, 자체 검사 예시뿐이다.
// 나머지는 Moja와 같게 둔다. 한쪽에서 고친 파서 결함은 다른 쪽에도 옮긴다.
//
...
/// cites 엣지는 테스트 주석도 본다. 요구사항 번호(F-xx)가 거기 적혀 있다.
...
/// 위키 문서를 새로 만들면 여기에 이름표를 더한다. 없으면 그 문서로 가는 참조가 조용히 사라진다
...
/// 식별자 계열을 늘리면(예: 수용 기준 T-01) 이 줄만 고친다.
let SYM_PREFIXES = "F|A"
/// 하이픈과 두 자리 숫자를 필수로 둔다. 메뉴바 앱 문서에 흔한 F1~F12(기능 키)나 A4 같은 낱말을
/// 심볼로 잡지 않기 위해서다.
let SYM = "(?:" + SYM_PREFIXES + ")-[0-9]{2}[a-z]?"
...
struct Section {
    let id: String          // "requirements#14.2a", "requirements#F-01"
    let doc: String
    let path: String        // kb/wiki 기준 상대 경로
    let level: Int          // 2 또는 3
    let num: String?        // "14.2a" / "F-01" / nil
    let heading: String
...
let SYMBOL_RE = Rx("(?<![A-Za-z0-9-])(" + SYM + ")(?![0-9A-Za-z-])")
/// "F-20~F-23", "A-01\~A-07" 같은 범위. 원문은 GitHub가 단일 물결표를 취소선으로 그리지 않도록
/// `\~`로 이스케이프해서 적는다. 그래서 역슬래시를 선택으로 받는다. 식별자 안에 이미 하이픈이
/// 있으므로 하이픈은 범위 구분자로 받지 않는다.
let RANGE_RE = Rx("(?<![A-Za-z0-9-])(" + SYM_PREFIXES + ")-([0-9]{2})\\\\?[~–](?:\\1-)?([0-9]{2})(?![0-9])")

let DECLARATION_SHAPES = [
    // ### F-01. 메모리 수집   /   ### A-04: 수치 검증  (지금은 안 쓰지만 모양을 열어 둔다)
    Rx("^[\\s>]*#{2,3}[ \t]+(" + SYM + ")[.:][ \t]"),
    // | F-01 | 기본 5초 간격(설정 가능 1\~30초)으로 ... |   요구사항 4장과 6장은 전부 이 모양이다
    Rx("^[\\s>]*\\|\\s*\\*{0,2}\\s*(" + SYM + ")\\s*\\*{0,2}\\s*\\|"),
    // - **F-01** ... / - F-01: ...  (지금은 안 쓰지만 모양을 열어 둔다)
    Rx("^[\\s>]*[-*]\\s*\\*\\*\\s*(" + SYM + ")"),
    Rx("^[\\s>]*[-*]\\s*(" + SYM + ")\\s*[:：—-]"),
]
...
func symbolsIn(_ text: String) -> [String] {
    ...
    for c in RANGE_RE.all(text) {
        // 두 자리 숫자라 범위는 최대 F-00~F-99다. 따로 상한을 두지 않는다.
        guard let a = Int(c[2]), let b = Int(c[3]), a <= b else { continue }
        for n in a...b { out.append(c[1] + "-" + (n < 10 ? "0\(n)" : "\(n)")) }
    }
...
// 하이픈 뒤 숫자도 빼야 한다. 안 그러면 "F-01의 동작"·"A-03 참조"가 requirements#01 참조가 된다.
...
// 마일스톤이 곧 개발 단계다. 요구사항 13장이 0~4단계를 정하고, 단계마다 릴리스 태그 하나로 끝난다.
// V 형식(V1.1-DIST-01)은 Moja와 파서를 맞춰 두려고 함께 받는다.
let TASK_RE = Rx("^\\s*-\\s*\\[([ xX])\\]\\s*((?:P[0-9]|V[0-9]+(?:\\.[0-9]+)?)-[A-Z]{2,6}-[0-9]{2})\\s*:\\s*(.*)$")
...
/// 주석 줄인가. cites는 주석에서만 캔다. 식별자가 F-01처럼 생겼을 수 있어서다.
...
///   /// 읽기 실패는 throw가 아니라 nil이다 (F-06, 요구사항 8장).
...
private let HEAD_SYM_RE = Rx("^#{1,3}[ \t]+(" + SYM + ")[.:]")
...
func cmdCheck() -> Never {
    ...
    // 선언된 곳이 없는 심볼. 오타나 지워진 요구사항을 가리키는 인용이 여기 걸린다.
...
func cmdSelftest() -> Never {
    ...
    check("프론트매터: parents.sections", (parents.first?.strs("sections") ?? []).joined(separator: "|"), "12.1|F-03")
    ...
    check("제목: F 선언", head("### F-01. 메모리 수집 (필수)"), "3|F-01|메모리 수집 (필수)")
    check("제목: A 선언 콜론", head("### A-04: 수치 검증"), "3|A-04|수치 검증")
    check("제목: 번호+글자", head("### 14.2a F-05의 \"I/O 대기\"는 macOS에 없다 ⚠️ 판단 필요"),
          "3|14.2a|F-05의 \"I/O 대기\"는 macOS에 없다 ⚠️ 판단 필요")
    check("제목: 2단계 번호", head("## 14. 결정과 검토 의견 (2026-09-17)"), "2|14|결정과 검토 의견 (2026-09-17)")
    check("제목: 콜론 구분", head("### 4.1 수집: Collector"), "3|4.1|수집: Collector")
    check("제목: 번호 없음", head("### 수집 항목 (README용, 앱 자체 기능 아님)"),
          "3|-|수집 항목 (README용, 앱 자체 기능 아님)")

    // 3. 슬러그
    check("슬러그: 한글 유지", slugify("수집 항목 (README용, 앱 자체 기능 아님)"), "수집-항목-readme용-앱-자체-기능-아님")
    ...
    // 4. 심볼 (\b 함정 회귀 고정)
    check("심볼: F-05와 (한글 뒤)", symbolsIn("F-05와 디스크").joined(separator: ","), "F-05")
    check("심볼: A-04에", symbolsIn("A-04에 걸림").joined(separator: ","), "A-04")
    check("심볼: 괄호 안 나열", symbolsIn("(F-02, A-01)").joined(separator: ","), "F-02,A-01")
    check("심볼: 큰 번호", symbolsIn("F-64 기준").joined(separator: ","), "F-64")
    check("심볼: 하이픈 뒤는 제외", symbolsIn("F-03-x").joined(separator: ","), "")
    check("심볼: 접두사 있으면 제외", symbolsIn("aF-03 NOTE-03").joined(separator: ","), "")
    check("심볼: 소수는 아님", symbolsIn("14.2a 참조").joined(separator: ","), "")
    check("심볼: 하이픈 없는 낱말은 아님", symbolsIn("기능 키 F1과 A4 용지, FR-3, T13").joined(separator: ","), "")
    check("심볼: 범위 확장", symbolsIn("F-20~F-23의 태깅").sorted().joined(separator: ","), "F-20,F-21,F-22,F-23")
    check("심볼: 이스케이프한 범위", String(symbolsIn("검증(A-01\\~A-07)").count), "7")
    ...
    check("선언: h3 마침표", declares("### F-01. 메모리 수집 (필수)"), "F-01")
    check("선언: h3 콜론", declares("### A-04: 수치 검증"), "A-04")
    check("선언: 표 첫 칸", declares("| F-01 | 기본 5초 간격(설정 가능 1\\~30초)으로 샘플링한다 | 검진 기간 기본 14일 |"), "F-01")
    check("선언: 표 헤더는 제외", declares("| 코드 | 요구사항 | 비고 |"), "-")
    check("선언: 일반 표 행 제외", declares("| 호환성 | 최소 macOS 14 |"), "-")
    check("선언: 본문 언급 제외", declares("F-03의 전제는 성립한다"), "-")

    // 6. 로드맵
    let tasks = parseRoadmap("- [x] P1-RULE-03: 메모리 판정 (CoreKit/Sources/Verdict/RuleEngine.swift) (14.3 참조) (2026-09-04)")
    if let t = tasks.first {
        check("로드맵: id", t.id, "P1-RULE-03")
        check("로드맵: 상태", t.status, "done")
        check("로드맵: 마일스톤/모듈", "\(t.milestone)/\(t.module)", "P1/RULE")
        check("로드맵: 날짜", t.date ?? "nil", "2026-09-04")
        check("로드맵: 코드 경로", t.codePaths.joined(separator: ","), "CoreKit/Sources/Verdict/RuleEngine.swift")
        check("로드맵: 절 참조", t.sectionRefs.map { "\($0.0)#\($0.1)" }.joined(separator: ","), "requirements#14.3")
    } else { fail += 1; print("❌ 로드맵: 한 줄도 못 읽음") }
    let symbolTask = parseRoadmap("- [ ] P2-AI-01: A-04의 자리표시자 검증 (요구사항 14.4)")
    check("로드맵: 심볼 뒤 조사는 절 참조가 아님",
          symbolTask.first.map { t in t.sectionRefs.map { "\($0.0)#\($0.1)" }.joined(separator: ",") } ?? "nil",
          "requirements#14.4")
    check("로드맵: 심볼", symbolTask.first?.symbols.joined(separator: ",") ?? "nil", "A-04")
    ...
    check("코드 경로: 한글 앞", orderedUnique(CODE_PATH_RE.all("측정은 scripts/probe-smc.swift로").map { $0[1] }).joined(separator: ","),
          "scripts/probe-smc.swift")
    ...
    check("참조: 이름표 + 장은 한 건", refs("요구사항 5장을 보라"), "requirements#5")
    check("참조: 한국어 이름표", refs("(요구사항 14.2a)"), "requirements#14.2a")
    check("참조: 한국어 단서", refs("14.2a 참조"), "requirements#14.2a")
    check("참조: 한국어 장", refs("5장 표에 반영"), "requirements#5")
    check("참조: 식별자 뒤 조사는 아님", refs("F-01의 동작과 A-03 참조"), "")
